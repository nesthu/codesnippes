create or replace PROCEDURE UPDATE_PN_ZU_RMATERAL_SEARCH AS
BEGIN

execute immediate 'truncate table HLP_PN_ZU_RMATERAL_SEARCH DROP STORAGE' ;

INSERT INTO HLP_PN_ZU_RMATERAL_SEARCH (
    LIEFERANTMATERIAL,
    MATERIAL_SEARCH
    )
...
from (
--Liefrantenmaterialnummer OEM Ursprung (Mehrfach) aus Klassifizierung für alle Standorte holen NUR R-Parts
        select distinct
...
--Liefrantenmaterialnummer aus Infosatz  für alle Standorte holen NUR R-Parts
        select distinct
        ...
        where LENGTH(i.LIEFERANTENMATERIALNR) > 3
            and r.WERKSSPEZ_MATERIALSTATUS = 'R'
            and trim(i.LIEFERANTENMATERIALNR) not in ('no OEM p/n','nicht bekannt','N.A.','OHNE','KEINE') -- Ausblenden nicht realer Nummern

union

--Liefrantenmaterialnummer OEM aktuell aus Klassifizierung für alle Standorte holen NUR R-Parts
        select distinct
            k.WERK ||'_'||k.OBJEKT,
...
--Liefrantenmaterialnummer  PARTNUMMER_LIEFARTNR_1 aus Klassifizierung für alle Standorte holen NUR R-Parts
        select distinct
            k.WERK ||'_'||k.OBJEKT,
...
--Liefrantenmaterialnummer PARTNUMMER_LIEFARTNR_2 aus Klassifizierung  für alle Standorte holen NUR R-Parts
        select distinct
            k.WERK ||'_'||k.OBJEKT,
...
END UPDATE_PN_ZU_RMATERAL_SEARCH;
